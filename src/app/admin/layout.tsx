  ...
  return (
    <div className="flex min-h-screen bg-[#f8fafc] dark:bg-slate-950 font-tajawal" dir="rtl">
      ...
      <aside className="w-72 bg-white dark:bg-slate-900 border-l border-slate-200 dark:border-slate-800 shadow-xl hidden lg:block z-30 sticky top-0 h-screen overflow-y-auto transition-all">
        <div className="p-8 pb-6">
          ...
          <div className="space-y-1">
            ...
            <SidebarLink href="/admin/products" icon={<Package className="w-5 h-5" />} label="المنتجات" />
        ...
        <div className="absolute bottom-0 w-full p-6 border-t border-slate-100 dark:border-slate-800">
          <Link href="/" className="flex items-center gap-3 w-full px-4 py-3 rounded-xl text-slate-600 hover:text-red-500 hover:bg-red-50 dark:text-slate-400 dark:hover:text-red-400 dark:hover:bg-red-500/10 transition-all font-bold">
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        {/* Top Header */}
        <header className="h-20 bg-red-600 lg:bg-white/80 dark:lg:bg-slate-900/80 backdrop-blur-md border-b border-slate-200 dark:border-slate-800 flex items-center justify-between px-8 sticky top-0 z-20">
          <div className="lg:hidden absolute left-1/2 -translate-x-1/2 font-black text-white text-sm bg-black px-2 py-1 rounded shadow-lg">
            نظام الجوال نشط ✓
          </div>
      ...
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 bg-white dark:bg-slate-900 border-t-2 border-primary/20 dark:border-slate-800 px-2 py-3 z-[100] flex justify-around items-center shadow-[0_-10px_30px_rgba(0,0,0,0.1)]">
        ...
        <MobileNavLink href="/admin/products" icon={<Package className="w-6 h-6" />} label="المنتجات" />
